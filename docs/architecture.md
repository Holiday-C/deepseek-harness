...
The long-lived Web surface runs its profile in a supervised worker. The parent keeps only process replacement and signal forwarding; every application capability remains in the worker's Cordis tree. A root-agent plugin can reserve one restart through the launcher-provided `appRestart`, end its turn, wait for application quiescence, and commit the private worker exit status. The parent then starts the same entry and arguments from the current artifacts. Other profiles remain direct because their stdio clients and one-shot lifecycle do not provide the Web client's reconnect semantics.
