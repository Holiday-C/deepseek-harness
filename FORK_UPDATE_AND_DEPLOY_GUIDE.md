...
在本仓库工作时，可以直接说“更新 DSH”“升级一下”或“同步官方新版”。[dsh-update skill](.agents/skills/dsh-update/SKILL.md) 按上下文识别更新意图并执行本手册，不要求记住文档名或工具名；仅询问版本或更新内容不会授权更新。明确要求只检查、只同步源码或不重启时，以该范围为准。

通过包含 supervisor 支持的 `dsh web` 启动后，可以让当前根 agent 按本手册完成源码同步、依赖安装、构建和检查，再调用 `restart_dsh` 交接到新进程。supervisor 留在 Cordis 应用树之外，因此 worker 构建自身源码时不会覆盖负责重新拉起它的已加载代码。旧版本尚未包含该能力时，仍需使用下文的人工停止与启动流程完成一次升级。

会话内更新必须保持下面的顺序：

1. 先执行[检查工作区](#2-检查工作区)与[记录当前版本](#3-记录当前版本)；需要备份用户数据的重大更新，应在启动本次 Web 会话前完成离线备份。
2. 按[同步官方仓库](#同步官方仓库)完成 `master` 与 `custom/main` 的同步和冲突处理，不要杀死或替换当前 DSH 进程。
3. 在旧 worker 仍提供基础工具时运行 `corepack pnpm install`、`corepack pnpm run build` 和本次变更需要的检查。构建或检查失败就留在旧进程中修复或回退，不调用重启工具。
4. 所有检查成功后调用 `restart_dsh`，向用户说明 Web 会短暂断开，并等待审批。工具会结束当前回合；另一个 agent 或后台 job 仍运行时会拒绝重启。
5. 浏览器自动重连后，按[重新部署验收](#重新部署验收)确认新 worker；若页面没有恢复，使用终端新打印的 `dsh web:` URL 手动打开。

`restart_dsh` 只负责安全交接，不会替代 Git、pnpm、构建、测试或冲突决策。它的成功结果表示重启已排到本回合结束，并不表示新 worker 已完成启动。若回合收尾期间出现新工作，预约会取消，旧进程继续运行；处理完工作后重新调用即可。

## 更新官方源码前

### 1. 人工更新时停止服务

没有 `restart_dsh`，或需要在更新前离线备份 `~/.dsh` 时，在运行 DSH 的终端按 `Ctrl+C`，确认进程已经退出。使用上一节的会话内更新且无需本次备份时，保留 Web 进程继续运行。
...
源码启动器不检查已有构建产物是否最新。运行完整 build 后调用 `restart_dsh`；旧版本没有该工具时，停止服务再启动。
