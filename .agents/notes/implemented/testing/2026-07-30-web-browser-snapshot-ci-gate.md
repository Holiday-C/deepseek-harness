...
CI's `scripts/run-web-snapshots.ts` first runs `hmr-live.e2e.ts`, `cordis-tool-round.e2e.ts`, and `workspace-management.e2e.ts` as separate serial Vitest invocations. The HMR scenario mutates built workspace state. The Cordis scenario owns a lifecycle-sensitive approval and steering sequence whose turn grouping is made deterministic by waiting for the initial turn to settle before approval. Workspace management owns one stateful browser and scaffold across registration, hover, menu, and archive transitions, so it runs without another browser file contending for the hosted runner. After all three pass, one bounded Vitest pool runs every remaining file. Every child inherits stdio, and the enclosing gate streams that output through `run-gates`.
...
**Run the stateful owners inside the parallel pool.** Rejected because HMR mutates shared built state, the Cordis approval continuation requires a serial preflight, and workspace management carries one browser state through ordered registry and interaction transitions. Every other file shares one bounded pool; dedicated long-file processes add scheduling code and leave part of a reduced worker budget idle after those files complete.
...
Before merge, every PR proves that the current web assembly matches all committed browser expected outputs; a missing refresh fails in the same PR that changes the assembly. The cost is Chromium provisioning, three serial scenarios, and one bounded pool in the consumer job; the consumer-owned build and browser cache avoid duplicate builds and downloads on reruns. Parallel-file failures stream immediately, but a worker-budget change still requires a completed end-to-end measurement rather than an elapsed-time guess. The gate makes no claim of cross-platform browser consistency, and if a Playwright/Chromium upgrade changes the ARIA format, the upgrade PR must explicitly refresh the expected outputs and review the churn.
