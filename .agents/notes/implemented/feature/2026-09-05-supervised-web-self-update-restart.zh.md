...
Web bundle 挂载重启 consumer，其他 profile 不挂载。应用启动架构、CLI README、boot 分组映射、`dsh-cmdline` README、生成目录和个人 Fork 更新手册会说明监督器、短暂重连窗口、显式审批，以及更新准备与重启提交之间的分工。
...
- 无密钥 Web 快照在 standard 与 minimal native schema 以及生成的 PTC SDK 提示词中固定 `restart_dsh`。
