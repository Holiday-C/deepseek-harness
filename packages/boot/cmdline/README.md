...
The launcher makes these values available to your app:
...
- `ctx.appRestart` — an optional exclusive restart reservation. `prepare()` fails while another request is reserved; its request can be committed or cancelled once, with repeat settlement doing nothing.
...
- **Launcher facts, not config.** `cmdlineArgs`, `appExit`, `appReady`, and optional `appRestart` are provided on the host context before the tree mounts; they are not Loader rows, so no composition owns or overrides them.
...
The parse path is one small family with two owners: `provideCmdline` freezes the host arguments and provides the launcher's lifecycle values before any tree entry mounts, and `parseCmdline` runs your commander program against the immutable arguments, routing every command's help, version, and error output through the launcher. A rejected value, `--help`, or `--version` prints commander's text and requests `ctx.appExit` without publishing anything, so dependent rows never activate; Loader defers each row's `!!js` interpolation until its declared injections are active. Per-export contracts live in the code, not this README — see [`src/index.ts`](src/index.ts).

### Source map

| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | `CmdlineArgs`/`AppExit` types, restart reservation, `provideCmdline`, `parseCmdline`, commander exit/output routing |
