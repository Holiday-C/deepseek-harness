...
运行 `dsh --profile web`，界面会在你的默认浏览器中打开，即可与 agent（智能体）交互式聊天。你会获得会话视图、模型与设置管理、会话历史，以及成功更新源码或重建后的受监督重启交接。该命令会打印带 token 的启动 URL；浏览器用该 token 换取签名会话 cookie，再重定向到干净的根 URL。你可以从命令行更改端口、关闭浏览器交接并允许额外主机；有意不支持绑定所有网络接口。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
正在运行的 agent 完成更新、构建与所需检查后，可以请求 `restart_dsh`。工具会请求审批、结束当前回合，并且只在没有其他 agent 或后台 job 运行时替换 Web worker。页面可能短暂显示断开，随后会连接到新进程；终端会打印新的启动 URL，供手动恢复。
...
本组合包是一份 patch 加一个运行时粘合插件。patch 重述 base 刻意省略的表层专属值，插入仅 Web 使用的宿主行与浏览器名录，挂载宿主持有的重启适配器，再把其余 agent 层移到 preset 之后；粘合插件负责 dist 服务、信任采样、提示词段落、bash 变量与就绪宣告。
